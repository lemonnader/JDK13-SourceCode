 ...
 * 对add(int, e)或remove(int)的单个调用向此字段添加的数量不得超过 1，否则迭代器（和列表迭代器）将抛出虚假的
...
public class ArrayList<E> extends AbstractList<E>
    ...
    private Object[] grow(int minCapacity) {
        int oldCapacity = elementData.length;//数组长度（capacity），添加第一个元素前为0
        if (oldCapacity > 0 || elementData != DEFAULTCAPACITY_EMPTY_ELEMENTDATA) {
            //新的容量是max（oldCapacity+额外需要的容量，1.5*oldCapacity）,一次到位，避免直接扩充为1.5倍不够。还要进行第二次
            int newCapacity = ArraysSupport.newLength(oldCapacity,
                    minCapacity - oldCapacity, /* minimum growth */
                    oldCapacity >> 1           /* preferred growth */);
            return elementData = Arrays.copyOf(elementData, newCapacity);
        } else {
            //第一次添加元素时，直接将一个容量为10（DEFAULT_CAPACITY）的数组赋值给elementData
            return elementData = new Object[Math.max(DEFAULT_CAPACITY, minCapacity)];
        }
    }

    private Object[] grow() {
        return grow(size + 1);//确保至少可以放下size+1个元素
    }
    ...
    public E get(int index) {
        ...
        return elementData(index);//直接按照索引返回数组内的该元素
    }
    ...
    private void add(E e, Object[] elementData, int s) {
        ...
        elementData[s] = e;
        size = s + 1;//每次加入一个元素之后，size就是下一个即将加入的元素的索引
    }
    ...
    public void add(int index, E element) {
        ...
        if ((s = size) == (elementData = this.elementData).length)
            elementData = grow();//会调用grow(size + 1)，确保可以放下size + 1个元素
        //src:源数组； srcPos:源数组要复制的起始位置； dest:目的数组； destPos:目的数组放置的起始位置； length:复制的长度
        // 将elementData从index位置开始，复制到elementData的index+1开始的连续空间
        System.arraycopy(elementData, index,
                elementData, index + 1,
                s - index);
        elementData[index] = element;//放进新元素
        size = s + 1;//长度变长
    }
     ...
     * 私有方法，跳过边界检查，并且不返回删除的值
     */
    private void fastRemove(Object[] es, int i) {
        modCount++;//快速失败
        final int newSize;
        if ((newSize = size - 1) > i)//i不是最后一个元素，后半部分向前copy
            System.arraycopy(es, i + 1, es, i, newSize - i);
        //最后的位置置null,同时适用于i是最后一个元素的情况
    ...
    public boolean addAll(Collection<? extends E> c) {
        ...
        Object[] elementData;
        final int s;
        if (numNew > (elementData = this.elementData).length - (s = size))//（capacity-size）空余位置不够
            elementData = grow(s + numNew);//扩容
        //将a中的所有元素复制到[s,s+numNew-1]的位置
     ...
     * 将目标容器里的所有元素插入ArrayList的index位置
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        ...
        int numMoved = s - index;//需要移动的元素个数；[index, s-1]
        if (numMoved > 0)
            //[index,s - 1] -> [index + numNew, s-1+numNew]
            System.arraycopy(elementData, index,
     ...
     * complement传入false，表示删除包含在c中的元素
     ...
     * 从该列表中删除所有未包含在指定集合中的元素；可以求两个集合的交集
    ...
    boolean batchRemove(Collection<?> c, boolean complement,
        ...
        for (r = from; ; r++) {
            if (r == end)
                //一直遍历到最后都没有发现有不重合的元素，说明两个集合元素相同
                return false;
            if (c.contains(es[r]) != complement)
                break;//找到第一个不同的元素
        }
        int w = r++;//写入开始的位置；前面的元素相同，不用动
        try {
            ...
            //出现异常，则把未读的元素都拷贝到写指针之后
            System.arraycopy(es, r, es, w, end - r);
            w += end - r;
            throw ex;
        } finally {
            modCount += end - w;//一共做了end - w次改动（删和改）
            shiftTailOverGap(es, w, end);//将写指针之后的元素置为空，帮助GC
        }
        return true;
    }
