                           ...
                           implements Deque<E>, Cloneable, Serializable
{
    ...
    private void grow(int needed) {
        ...
        //数组长度小于64，就直接两倍，否则长度变为原来的1.5倍
        int jump = (oldCapacity < 64) ? (oldCapacity + 2) : (oldCapacity >> 1);
        if (jump < needed//扩容完还不够，或者超过了最大容量
        ...
        if (tail < head || (tail == head && es[head] != null)) {
            ...
            //如果tail到了head的前面或者两者相等
            int newSpace = newCapacity - oldCapacity;//多出来的容量
    ...
    public ArrayDeque(int numElements) {
        elements =
            new Object[(numElements < 1) ? 1 ://传入的是0的话，就把容量初始化为1
                       (numElements == Integer.MAX_VALUE) ? Integer.MAX_VALUE ://超过边界，容量初始化为最大值
                       numElements + 1];//正常情况下初始化为多一个
    }
     ...
     * Precondition and postcondition: 0 <= i < modulus.
     */
    //循环递减i;i-1小于0的话，就变为modulus - 1；modulus一般为数组长度
    ...
    static final int sub(int i, int j, int modulus) {
        //如果head(j)在右边，tail(i)在左边，那么真实的元素个数就是(modulus - head) + tail,也就是modulus+(tail - head)
        if ((i -= j) < 0) i += modulus;
        return i;
    }
    ...
    public void addFirst(E e) {
        ...
        //head指针上移；循环-1【为了防止溢出】；head指向的是当前的队首元素
        es[head = dec(head, es.length)] = e;
        if (head == tail)//循环数组满了
            grow(1);//如果头尾挨在一起了，就扩容；新添加了1个元素，needed（需要增加的容量）为1
    }
    ...
    public void addLast(E e) {
        if (e == null)// 不允许null元素
            throw new NullPointerException();
        final Object[] es = elements;
        es[tail] = e;//tail指向的是最后一个元素的后面
        if (head == (tail = inc(tail, es.length)))//添加新元素后tail要循环+1
            grow(1);//头尾碰上了就要扩容
    }
    ...
    public E pollLast() {
        final Object[] es;
        final int t;
        // 取当前尾元素【注意tail指向的是尾元素的后一位，所以要先循环-1】
        E e = elementAt(es = elements, t = dec(tail, es.length));
        if (e != null)
            es[tail = t] = null;//尾元素处置为null并更新tail
        return e;
    }
