...
public class CopyOnWriteArraySet<E> extends AbstractSet<E>
     ...
     */
    public CopyOnWriteArraySet(Collection<? extends E> c) {// 将集合c中的元素初始化到CopyOnWriteArraySet中
    ...
    public boolean containsAll(Collection<?> c) {
        ...
        return (c instanceof Set)
            ...
            : al.containsAll(c);//containsAll方法需要拿c的每个元素在al里查找位置（O(N)），时间复杂度为O(N^2)
    }
     ...
     */
    private static int compareSets(Object[] snapshot, Set<?> set) {   //降低了时间复杂度
        ...
        final boolean[] matched = new boolean[len];//利用一个matched数组标记已经匹配上的元素
        //空间换时间
        ...
        outer: for (Object x : set) {
            for (int i = j; i < len; i++) {
                if (!matched[i] && Objects.equals(x, snapshot[i])) {
                    matched[i] = true;//snapshot[i]已经匹配上了，就标记为true；这样已经匹配上的元素就不用走下面的代码了
                    if (i == j)
                        do { j++; } while (j < len && matched[j]);//这里应该是认为i的右边可能还有已经匹配上的元素，所以能跳多少算多少
                    continue outer;
                }
            }
            return -1;
     ...
     * @see #add(Object)
     */
    public boolean addAll(Collection<? extends E> c) {    // 并集
    ...
    public boolean equals(Object o) {// equals()方法
        return (o == this)//是否是同一个对象
            || ((o instanceof Set)
                && compareSets(al.getArray(), (Set<?>) o) == 0);//如果c是set，比较内容是否相同
    }

    /**
     * @throws NullPointerException {@inheritDoc}
     */
    public boolean removeIf(Predicate<? super E> filter) {// 移除满足过滤条件的元素
     ...
     * @since 1.8
     */
    public Spliterator<E> spliterator() {// 分割的迭代器
