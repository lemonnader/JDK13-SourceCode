...
public class PriorityBlockingQueue<E> extends AbstractQueue<E>
     ...
     */
    private transient Object[] queue;// 存储元素的地方
     ...
     */
    private transient volatile int allocationSpinLock;// 扩容的时候使用的控制变量，CAS更新这个值，谁更新成功了谁扩容，其它线程让出CPU
     ...
     */
    private PriorityQueue<E> q;// 不阻塞的优先级队列，非存储元素的地方，仅用于序列化/反序列化时
     ...
     *         than 1
     */
    public PriorityBlockingQueue(int initialCapacity) {// 传入初始容量
        ...
        if (allocationSpinLock == 0 &&
            ...
            try {
                ...
                if (newCap - MAX_ARRAY_SIZE > 0) {    // possible overflow
                    ...
                    newCap = MAX_ARRAY_SIZE;// 如果没越界，则设置为最大值
                }
                if (newCap > oldCap && queue == array)
                    newArray = new Object[newCap]; // 创建新数组
            } finally {
                allocationSpinLock = 0;// 相当于解锁
            }
        }
    ...
    private E dequeue() {
        ...
        if ((result = (E) ((es = queue)[0])) != null) {//数组长度大于0；result是堆顶元素
            final int n;
            final E x = (E) es[(n = --size)]; // 元素个数减1
            es[n] = null;//弹出堆尾元素
            if (n > 0) {
    ...
    private static <T> void siftUpComparable(int k, T x, Object[] es) {
        ...
        while (k > 0) {
            // 取父节点
            int parent = (k - 1) >>> 1;
            Object e = es[parent];// 父节点的元素值
            if (key.compareTo((T) e) >= 0) // 如果key大于父节点，堆化结束
                break;
            es[k] = e;// 否则，交换二者的位置，继续下一轮比较
            k = parent;
        }
        es[k] = key;// 找到了应该放的位置，放入元素
    }
    ...
    public E take() throws InterruptedException {
        ...
        try {
            while ( (result = dequeue()) == null)
                notEmpty.await();// 队列没有元素，就阻塞在notEmpty条件上
            // 出队成功，就跳出这个循环
        } finally {
            lock.unlock(); // 解锁
        }
