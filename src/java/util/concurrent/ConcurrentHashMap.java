...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    private static final int DEFAULT_CAPACITY = 16;// 默认初始值，必须是2的幂数
     ...
     * shrinkage.
     */
    static final int TREEIFY_THRESHOLD = 8;// 链表转红黑树阈值,> 8 链表转换为红黑树
     ...
     */
    private transient volatile long baseCount;//该属性保存着整个哈希表中存储的所有的结点的个数总和，有点类似于 HashMap 的 size 属性
     ...
     */
    private transient volatile int sizeCtl;//控制标识符，用来控制table的初始化和扩容的操作，不同的值有不同的含义
    ...
    public V get(Object key) {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            (e = tabAt(tab, (n - 1) & h)) != null) {// 如果元素所在的桶存在且里面有元素
            if ((eh = e.hash) == h) {// 如果第一个元素就是要找的元素，直接返回
                if ((ek = e.key) == key || (ek != null && key.equals(ek)))
                    return e.val;
            }
            else if (eh < 0)// hash小于0，说明是树或者正在扩容
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        if (key == null || value == null) throw new NullPointerException(); // key和value都不能为null
        // 计算hash值:(h ^ (h >>> 16)) & HASH_BITS【0x7fffffff】
        //将Key的hashCode值与其高16位作异或并保证最高位为0（从而保证最终结果为正整数）。
        int hash = spread(key.hashCode());
        int binCount = 0; // 要插入的元素所在桶的元素个数
        for (Node<K,V>[] tab = table;;) {// 死循环，结合CAS使用（如果CAS失败，则会重新取整个桶进行下面的流程）
            Node<K,V> f; int n, i, fh; K fk; V fv;
            if (tab == null || (n = tab.length) == 0)
                tab = initTable(); // 如果桶未初始化或者桶个数为0，则初始化桶
                ...
                //【如果在进行扩容转移中(当前节点是forwardingNode)】，则当前线程帮忙一起迁移元素
                ...
                // 如果这个桶不为空且不在迁移元素，则锁住这个桶（分段锁的概念）
                // 并查找要插入的元素是否在这个桶中
                // 存在，则替换值（onlyIfAbsent=false，true的时候不替换）
                // 不存在，则插入到链表结尾或插入树中
                V oldVal = null;
                synchronized (f) {
        ...
        addCount(1L, binCount);// 成功插入元素，元素个数加1（是否要扩容在这个里面）
    ...
    final V replaceNode(Object key, V value, Object cv) {
        ...
        for (Node<K,V>[] tab = table;;) {// 自旋
            ...
            if (tab == null || (n = tab.length) == 0 ||
                ...
                break; // 如果目标key所在的桶不存在，跳出循环返回null
            else if ((fh = f.hash) == MOVED)
                tab = helpTransfer(tab, f);// 如果正在扩容中，协助扩容
            else {
                V oldVal = null;
                ...
                synchronized (f) {
                    if (tabAt(tab, i) == f) {// 再次验证当前桶第一个元素是否被修改过
                        if (fh >= 0) {// fh>=0表示是链表节点
                                ...
                                if (e.hash == hash &&
                                    ...
                                    if (cv == null || cv == ev ||
                                        ...
                                        if (value != null)
                                            ...
                                            setTabAt(tab, i, e.next);// 如果前置节点为空
                                        // 说明是桶中第一个元素，删除之
                                    }
                                    break;
                                }
                            ...
                            TreeNode<K,V> r, p;
                            if ((r = t.root) != null &&
                                (p = r.findTreeNode(hash, key, null)) != null) {// 遍历树找到了目标节点
                                V pv = p.val;
                                if (cv == null || cv == pv ||
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            if ((sc = sizeCtl) < 0)
                // 如果sizeCtl<0说明有其他线程正在初始化或者扩容，让出CPU
                ...
                try {
                    // 再次检查table是否为空，防止ABA问题：其他线程完成初始化以后又进行了其他操作将sc改回去了
    ...
    private final void addCount(long x, int check) {
        ...
        if (check >= 0) {
            ...
            while (s >= (long)(sc = sizeCtl) && (tab = table) != null &&
                ...
                int rs = resizeStamp(n) << RESIZE_STAMP_SHIFT;// rs是扩容时的一个邮戳标识
                if (sc < 0) {// sc<0说明正在扩容中
                ...
                synchronized (f) {
                    ...
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) {
                            ...
                            // 标记当前桶已迁移
                            setTabAt(tab, i, fwd);
                            advance = true;// advance为true，返回上面进行--i操作
                        }
                        else if (f instanceof TreeBin) {
                            // 如果第一个元素是树节点
                            // 也是一样，分化成两颗树
