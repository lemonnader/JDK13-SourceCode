...
public class LinkedBlockingQueue<E> extends AbstractQueue<E>
    ...
    private E dequeue() {
        ...
        Node<E> h = head;// head节点本身是不存储任何元素的
    ...
    public LinkedBlockingQueue(int capacity) {
        ...
        last = head = new Node<E>(null);// 初始化head和last指针为空值节点
    }
    ...
    public void put(E e) throws InterruptedException {
        ...
        final int c;
        final Node<E> node = new Node<E>(e);// 新建一个节点
        final ReentrantLock putLock = this.putLock;
        ...
        try {
            ...
            c = count.getAndIncrement();// 队列长度加1【原子操作】
            // 如果现队列长度如果小于容量
            // 就再唤醒一个阻塞在notFull条件上的线程
            // 这里为啥要唤醒一下呢？
            // 因为可能有很多线程阻塞在notFull这个条件上的
            // 而取元素时只有取之前队列是满的才会唤醒notFull【看take方法的源码】
            // 为什么队列满的才唤醒notFull呢？
            // 因为唤醒是需要加putLock的，这是为了减少锁的次数
            // 所以，这里索性在放完元素就检测一下，未满就唤醒下一个notFull上的线程
            // 说白了，这也是锁分离带来的代价
            if (c + 1 < capacity)
                notFull.signal();
        } finally {
    ...
    public E take() throws InterruptedException {
        ...
        try {
            ...
            x = dequeue();// 否则，出队
            c = count.getAndDecrement(); // 获取出队前队列的长度并将队列长度减1【原子操作】
            if (c > 1)
                notEmpty.signal();// 如果取之后队列长度大于1，则唤醒下一个等待notEmpty的线程
        } finally {
            takeLock.unlock(); // 释放锁
        }
        if (c == capacity)// 如果取之前队列长度等于容量
            // 则唤醒notFull
            signalNotFull();
        return x;
    }
