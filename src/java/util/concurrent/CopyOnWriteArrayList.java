...
public class   CopyOnWriteArrayList<E>
    ...
    public CopyOnWriteArrayList(Collection<? extends E> c) {
        ...
        if (c.getClass() == CopyOnWriteArrayList.class)
            ...
            es = c.toArray();// 否则调用其toArray()方法将集合元素转化为数组
            // defend against c.toArray (incorrectly) not returning Object[]
            // (see e.g. https://bugs.openjdk.java.net/browse/JDK-6260652)
            if (es.getClass() != Object[].class)// 这里c.toArray()返回的不一定是Object[]类型
                // 详细原因见ArrayList里面的分析
    ...
    public void add(int index, E element) {
        synchronized (lock) {// 加锁
            Object[] es = getArray();// 获取旧数组
            int len = es.length;
            if (index > len || index < 0)// 检查是否越界, 可以等于len
                throw new IndexOutOfBoundsException(outOfBounds(index, len));
            Object[] newElements;
            int numMoved = len - index;
            if (numMoved == 0)// 如果插入的位置是最后一位
    ...
    public E remove(int index) {
        synchronized (lock) {// 加锁
            ...
            Object[] newElements;
            if (numMoved == 0)// 如果移除的是最后一位
                // 那么直接拷贝一份n-1的新数组, 最后一位就自动删除了
                newElements = Arrays.copyOf(es, len - 1);
            else {
                // 如果移除的不是最后一位,那么新建一个n-1的新数组
                newElements = new Object[len - 1];
    ...
    public boolean addIfAbsent(E e) {
        Object[] snapshot = getArray();// 获取元素数组, 取名为快照
        // 检查如果元素存在,直接返回false
        // 如果不存在再调用addIfAbsent()方法添加元素
        //indexOfRange在一定范围内查找元素e，找到了就返回其坐标，找不到就返回-1
    ...
    private boolean addIfAbsent(E e, Object[] snapshot) {
        synchronized (lock) {// 加锁
            Object[] current = getArray();// 重新获取旧数组
            int len = current.length;
            if (snapshot != current) {// 如果快照与刚获取的数组不一致，说明有修改
                // Optimize for lost race to another addXXX operation
                // 重新检查元素是否在刚获取的数组里
                int common = Math.min(snapshot.length, len);//公共部分里面找
                for (int i = 0; i < common; i++)//
                    if (current[i] != snapshot[i]
                        && Objects.equals(e, current[i]))
                        return false;//里面已经有e了
                if (indexOfRange(e, current, common, len) >= 0)//多出来的那部分找
                        return false;
            }
     ...
     */
    public int addAllAbsent(Collection<? extends E> c) {//如果元素不存在，就添加
        Object[] cs = c.toArray();
        if (cs.length == 0)
            return 0;
        synchronized (lock) {//加锁
            ...
            for (int i = 0; i < cs.length; ++i) {
                Object e = cs[i];
                //indexOfRange的时间复杂度为O(N)，遍历整个数组查找目标对象
                if (indexOfRange(e, es, 0, len) < 0 &&
                    indexOfRange(e, cs, 0, added) < 0)
                    cs[added++] = e;//原数组和要添加的新数组中都没有元素e，就把它保留下来；这个地方是直接覆盖了
            }
            if (added > 0) {
                //新建数组，长度为旧数组长度和新添加的元素个数的和；同时把原数组元素拷贝进去
                Object[] newElements = Arrays.copyOf(es, len + added);
                System.arraycopy(cs, 0, newElements, len, added);//把新添加的元素放在后面
