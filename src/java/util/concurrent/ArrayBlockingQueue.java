...
public class ArrayBlockingQueue<E> extends AbstractQueue<E>
    ...
    private void enqueue(E e) {
        ...
        items[putIndex] = e; // 把元素直接放在放指针的位置上
        if (++putIndex == items.length) putIndex = 0;// 如果放指针到数组尽头了，就返回头部
        count++;// 数量加1
    ...
    private E dequeue() {
        ...
        final Object[] items = this.items;
        @SuppressWarnings("unchecked")
        E e = (E) items[takeIndex];// 取取指针位置的元素
        items[takeIndex] = null;// 把取指针位置设为null
        if (++takeIndex == items.length) takeIndex = 0;// 取指针前移，如果数组到头了就返回数组前端循环利用
        count--;// 元素数量减1
        if (itrs != null)
    ...
    public ArrayBlockingQueue(int capacity, boolean fair) {
        ...
        // 创建重入锁及两个条件
        lock = new ReentrantLock(fair);//fair控制重入锁的类型是公平锁还是非公平锁
    ...
    public void put(E e) throws InterruptedException {
        ...
        final ReentrantLock lock = this.lock;
        lock.lockInterruptibly();// 加锁，如果线程中断了抛出异常而不是继续等待锁
        try {
            // 如果数组满了，使用notFull等待
            ...
            // 这时候需要再次等待，必须要满足被唤醒同时队列没满才可以入队
            while (count == items.length)
                notFull.await();//队列是满的，调用Condition的await()方法，使当前【生产者】线程阻塞等待;
            //Condition被唤醒后会从await方法的下一句开始执行
            enqueue(e);// 入队
        } finally {
            lock.unlock();//解锁
        }
    ...
    public boolean offer(E e, long timeout, TimeUnit unit)
        ...
        try {
            while (count == items.length) {
                if (nanos <= 0L)
                    return false;
                nanos = notFull.awaitNanos(nanos);// 如果数组满了，就阻塞nanos纳秒
    ...
    public E take() throws InterruptedException {
        ...
        try {
            while (count == 0)
                notEmpty.await();// 如果队列无元素，则阻塞等待在条件notEmpty上
