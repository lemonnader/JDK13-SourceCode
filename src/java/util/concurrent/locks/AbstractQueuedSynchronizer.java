...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
        ...
        static final int CANCELLED =  1;// 标识线程已取消
         ...
         */
        final Node predecessor() { // 获取前一个节点
            Node p = prev;
            if (p == null)
    ...
    private Node addWaiter(Node mode) {
        Node node = new Node(mode);// 新建一个节点
        ...
        for (;;) {//自旋
            Node oldTail = tail;
            if (oldTail != null) {// 如果尾节点不为空
                node.setPrevRelaxed(oldTail);// 设置新节点的前置节点为现在的尾节点
                if (compareAndSetTail(oldTail, node)) {// CAS更新尾节点为新节点
                    oldTail.next = node;// 如果成功了，把旧尾节点的下一个节点指向新节点
                    return node;// 并返回新节点
                }
            } else {
                initializeSyncQueue();//尾节点为空，说明队列还没初始化，调用初始化函数
            }
        }
    }
    ...
    private void unparkSuccessor(Node node) {
        ...
        if (s == null || s.waitStatus > 0) { // 如果下一个节点为空，或者其等待状态大于0（实际为已取消）
            s = null;
    ...
    private final boolean parkAndCheckInterrupt() {
        ...
        return Thread.interrupted(); // 返回是否已中断
    }
        ...
        try {
            for (;;) {// 自旋
                ...
                if (shouldParkAfterFailedAcquire(p, node))
                    //如果确实要阻塞，parkAndCheckInterrupt才是真正的阻塞方法，同时检查现成的中断情况
                    interrupted |= parkAndCheckInterrupt();//如果中断了parkAndCheckInterrupt会返回true
            }
        } catch (Throwable t) {
            cancelAcquire(node);// 出现异常，取消获取锁
            if (interrupted)
                selfInterrupt();//被中断
            throw t;
        }
    ...
    private boolean doAcquireNanos(int arg, long nanosTimeout)
            throws InterruptedException {
        if (nanosTimeout <= 0L)// 如果时间已经到期了，直接返回false
            return false;
        ...
        try {
            for (;;) {
                ...
                if (shouldParkAfterFailedAcquire(p, node) &&
                    ...
                    LockSupport.parkNanos(this, nanosTimeout);// 阻塞一段时间
    ...
    public final void acquire(int arg) {
        // 尝试获取锁
        // 如果失败了，就排队
        if (!tryAcquire(arg) &&// 注意addWaiter()这里传入的节点模式为独占模式
            acquireQueued(addWaiter(Node.EXCLUSIVE), arg))
            //没获取到锁，并且排队的时候被中断了，执行线程的中断
            selfInterrupt();
    }
    ...
    public final boolean tryAcquireNanos(int arg, long nanosTimeout)
            throws InterruptedException {
        if (Thread.interrupted())// 如果线程中断了，抛出异常
            throw new InterruptedException();
        return tryAcquire(arg) ||// 先尝试获取一次锁
    ...
    public final boolean release(int arg) {
        if (tryRelease(arg)) {// 调用AQS实现类的tryRelease()方法释放锁
            Node h = head;
    ...
    final boolean isOnSyncQueue(Node node) {
        if (node.waitStatus == Node.CONDITION || node.prev == null)
            return false;// 如果等待状态是CONDITION，或者前一个指针为空，返回false
        // 说明还没有移到AQS的队列中
        if (node.next != null) // If has successor, it must be on queue
            return true;// 如果next指针有值，说明已经移到AQS的队列中了
        /*
         ...
         */
        return findNodeFromTail(node);// 从AQS的尾节点开始往前寻找看是否可以找到当前节点，找到了也说明已经在AQS的队列中了
    }
    ...
    final boolean transferForSignal(Node node) {
        ...
        Node p = enq(node);
        int ws = p.waitStatus;// 上一个节点的等待状态
    ...
    final int fullyRelease(Node node) {
        try {
            int savedState = getState();// 获取状态变量的值，重复获取锁，这个值会一直累加
            // 所以这个值也代表着获取锁的次数
            if (release(savedState))// 一次性释放所有获得的锁
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private Node addConditionWaiter() {
            if (!isHeldExclusively())
                throw new IllegalMonitorStateException();
            Node t = lastWaiter;// 如果条件队列的尾节点已取消，从头节点开始清除所有已取消的节点
            ...
            Node node = new Node(Node.CONDITION);// 新建一个节点，它的等待状态是CONDITION

            if (t == null) // 如果尾节点为空，则把新节点赋值给头节点（相当于初始化队列）
                // 否则把新节点赋值给尾节点的nextWaiter指针
                firstWaiter = node;
            else
                t.nextWaiter = node; // 尾节点指向新节点
            lastWaiter = node;
            return node;// 返回新节点
        }
        ...
        private void doSignal(Node first) {
            do {  // 移到条件队列的头节点往后一位
                if ( (firstWaiter = first.nextWaiter) == null)
                    lastWaiter = null;
                first.nextWaiter = null; // 相当于把头节点从队列中出队
            } while (!transferForSignal(first) &&// 转移节点到AQS队列中
        ...
        public final void signal() {
            if (!isHeldExclusively())// 如果不是当前线程占有着锁，调用这个方法抛出异常
                // 说明signal()也要在获取锁之后执行
                throw new IllegalMonitorStateException();
            Node first = firstWaiter;// 条件队列的头节点
            if (first != null) // 如果有等待条件的节点，则通知它条件已成立
                doSignal(first);
        }
        ...
        public final void await() throws InterruptedException {
            if (Thread.interrupted())// 如果线程中断了，抛出异常
                throw new InterruptedException();
            Node node = addConditionWaiter(); // 添加节点到Condition的队列中，并返回该节点
            int savedState = fullyRelease(node);// 完全释放当前线程获取的锁
