...
public class ReentrantLock implements Lock, java.io.Serializable {
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        final boolean nonfairTryAcquire(int acquires) {
            ...
            if (c == 0) {
                ...
                // 相对于公平锁模式少了!hasQueuedPredecessors()条件,不用按照顺序，直接抢锁
        ...
        protected final boolean tryRelease(int releases) {
            ...
            if (Thread.currentThread() != getExclusiveOwnerThread())
                throw new IllegalMonitorStateException();// 如果当前线程不是占有着锁的线程，抛出异常
            boolean free = false;
            if (c == 0) {
    ...
    static final class FairSync extends Sync {
        ...
        protected final boolean tryAcquire(int acquires) {
            // 当前线程
            final Thread current = Thread.currentThread();
            int c = getState();// 查看当前状态变量的值
            if (c == 0) {// 如果状态变量的值为0，说明暂时还没有人占有锁
                // 如果没有其它线程在排队，那么当前线程尝试更新state的值为1
                // 如果成功了，则说明当前线程获取了锁
                if (!hasQueuedPredecessors() &&
                    compareAndSetState(0, acquires)) {
                    setExclusiveOwnerThread(current);
                    // 当前线程获取了锁，把自己设置到exclusiveOwnerThread变量中【独占锁】
            ...
            // 那么，直接让它获取锁并返回true
                ...
                int nextc = c + acquires;// 状态变量state的值加1【acquires为1】
                if (nextc < 0)// 如果溢出了，则报错
                    throw new Error("Maximum lock count exceeded");
                // 设置到state中
                // 这里不需要CAS更新state
     ...
     */
    public ReentrantLock(boolean fair) {// 自己可选择使用公平锁还是非公平锁
