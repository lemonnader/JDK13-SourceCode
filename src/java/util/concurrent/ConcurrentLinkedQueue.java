...
public class ConcurrentLinkedQueue<E> extends AbstractQueue<E>
     ...
     */
    transient volatile Node<E> head;// 链表头节点
    ...
    public ConcurrentLinkedQueue(Collection<? extends E> c) {
        ...
        for (E e : c) {
            Node<E> newNode = new Node<E>(Objects.requireNonNull(e));//不允许添加null元素
            if (h == null)
                h = t = newNode;
            else
                t.appendRelaxed(t = newNode);
        }
        if (h == null)//c是空集合时确保ConcurrentLinkedQueue的初始化
            h = t = new Node<E>();
        head = h;
        tail = t;
    }
    ...
    public boolean offer(E e) {
        //解释代码在多线程中的意义：https://thinkwon.blog.csdn.net/article/details/102508089
        final Node<E> newNode = new Node<E>(Objects.requireNonNull(e)); // 不能添加空元素

        for (Node<E> t = tail, p = t;;) {// 入队到链表尾
            Node<E> q = p.next;
            if (q == null) {// 如果没有next，说明到链表尾部了，就入队
                // p is last node
                if (NEXT.compareAndSet(p, null, newNode)) {
                    ...
                    // CAS更新p的next为新节点
                    // 如果成功了，就返回true
                    // 如果不成功就重新取next重新尝试【说明有其他线程添加了新节点，使得p的next不是null了】
                    if (p != t) // hop two nodes at a time; failure is OK
                        // 如果p不等于t，说明有其它线程先一步更新tail
                        // 但是当前线程更新尾节点的next是成功的，也就是其他线程走的不是q==null这个分支
                        // p取到的可能是tail后面的值
                        // 把tail原子更新为新节点
        ...
        restartFromHead: for (;;) {//出队是从头部出的
            ...
            for (Node<E> h = head, p = h, q;; p = q) {
                ...
                if ((item = p.item) != null && p.casItem(item, null)) {
                    ...
                    return item; // 上面的casItem()成功，就可以返回出队的元素了
                }
                // 下面三个分支说明头节点不对
                // 且p的item肯定为null
                    ...
                    continue restartFromHead; // 将p设置为p的next
            }
        }
    }
