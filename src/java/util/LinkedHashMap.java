    ...
    implements Map<K,V>
{
    ...
    // newNode 新节点，放到双向链表的尾部
    private void linkNodeLast(LinkedHashMap.Entry<K,V> p) {
        LinkedHashMap.Entry<K,V> last = tail;// 添加元素之前的双向链表尾部节点
        tail = p;// tail 指向新添加的节点
        if (last == null)
            head = p;//如果之前 tail 指向 null 那么集合为空新添加的节点 head = tail = p
        else {
            p.before = last;// 否则将新节点的 before 引用指向之前当前链表尾部
            last.after = p; // 之前链表尾部节点的 after 指向新节点
        }
    }
    ...
    void afterNodeRemoval(Node<K,V> e) { // unlink
        ...
        if (b == null)// p.before 为 null，表明 p 是头节点
            head = a;
        else//否则将 p 的前驱节点连接到 p 的后驱节点
            b.after = a;
        if (a == null)// a 为 null，表明 p 是尾节点
            tail = b;
...
// 如果accessOrder为true，并且访问的节点不是尾节点
        if (accessOrder && (last = tail) != e) {
            LinkedHashMap.Entry<K,V> p =
                (LinkedHashMap.Entry<K,V>)e, b = p.before, a = p.after;
            p.after = null;//访问节点的后驱置为 null
            if (b == null)//如访问节点的前驱为 null 则说明 p 为头节点
                head = a;//head指向头节点的下一个
            else//否则就让p的前驱结点的after指向p的after
                b.after = a;
            //如果 p 不为尾节点 那么将p的后置结点的before指向p的前驱结点
            if (a != null)
                a.before = b;
            else//p是尾结点，last结点指向尾结点的前驱节点
                last = b;
            //这里last==null肯定是因为上一句导致的，也就是a和b都等于null，只有p一个结点【好像不大可能】
            if (last == null)
                head = p;
            else {//p成为尾结点
                p.before = last;
