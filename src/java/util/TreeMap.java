    ...
    implements NavigableMap<K,V>, Cloneable, java.io.Serializable
{
    ...
    final Entry<K,V> getEntry(Object key) {
        ...
        while (p != null) {
            ...
            if (cmp < 0)// k的值如果小于根结点的值：从左子树查找
                p = p.left;
            else if (cmp > 0)// k的值如果大于根结点的值：从右子树查找
                p = p.right;
            else
    ...
    final Entry<K,V> getEntryUsingComparator(Object key) {
        ...
        if (cpr != null) {
            ...
            Entry<K,V> p = root;
            while (p != null) {
                //利用comparator的compare方法来判断，下面的判断方法同上
    ...
    public V put(K key, V value) {
        ...
        Comparator<? super K> cpr = comparator;
        if (cpr != null) {
            do {// 如果使用的是comparator方式，key值可以为null，只要在comparator.compare()中允许即可
                // 从根节点开始遍历寻找
                parent = t;
                cmp = cpr.compare(key, t.key);
                if (cmp < 0)
                    t = t.left;// 如果小于0从左子树寻找
                else if (cmp > 0)
                    t = t.right;// 如果大于0从右子树寻找
                else// 如果等于0，说明插入的节点已经存在了，直接更换其value值并返回旧值
                    return t.setValue(value);
            } while (t != null);
        }
        else {// 如果使用的是Comparable方式，key不能为null
            if (key == null)
                throw new NullPointerException();
            @SuppressWarnings("unchecked")
                Comparable<? super K> k = (Comparable<? super K>) key;
        ...
        if (cmp < 0)
            ...
            parent.right = e;
        fixAfterInsertion(e);// 插入之后的平衡
        size++;// 元素个数加1（不需要扩容）
        modCount++;// 修改次数加1
        return null;// 如果插入了新节点返回空
    }
    ...
    public void forEach(BiConsumer<? super K, ? super V> action) {
        ...
        for (Entry<K, V> e = getFirstEntry(); e != null; e = successor(e)) {
            action.accept(e.key, e.value);// 执行动作

            if (expectedModCount != modCount) {// 如果发现修改次数变了，则抛出异常
    ...
    private void rotateLeft(Entry<K,V> p) {
        if (p != null) {
            ...
            // （4）...
            if (p.parent == null)
                // 如果 x的父节点为空，说明x是根节点，则将y设置为根节点
                root = r;
            else if (p.parent.left == p)
                // 如果x是它父节点的左节点，则将y设置为x父节点的左节点
                p.parent.left = r;
            else// 如果x是它父节点的右节点，则将y设置为x父节点的右节点
                p.parent.right = r;
            r.left = p;// （5）将 x 设为 y的左节点
            p.parent = r;// （6）将 x的父节点 设为 y
        }
    }
    ...
    private void rotateRight(Entry<K,V> p) {
        if (p != null) {
            Entry<K,V> l = p.left;// p的左节点l，即x
            p.left = l.right;// （1）将 x的右节点 设为 y的左节点
            ...
            // （4）...
            if (p.parent == null)
            // 如果 y的父节点 是 空节点，说明y是根节点，则将x设为根节点
                root = l;
    ...
    private void deleteEntry(Entry<K,V> p) {
        ...
        if (replacement != null) {
            ...
            p.left = p.right = p.parent = null;// 将p的各项属性都设为空
            ...
            if (p.color == BLACK)// 如果当前节点没有子节点且其为黑节点，则把自己当作虚拟的替换节点进行再平衡
                fixAfterDeletion(p);

            if (p.parent != null) {// 平衡完成后删除当前节点（与父节点断绝关系）
                if (p == p.parent.left)
                    p.parent.left = null;
        ...
        while (x != root && colorOf(x) == BLACK) {
            if (x == leftOf(parentOf(x))) {
                ...
                if (colorOf(leftOf(sib))  == BLACK &&
                    ...
                    // 情况4）
                    setColor(sib, colorOf(parentOf(x)));// （1）将兄弟节点的颜色设为父节点的颜色
